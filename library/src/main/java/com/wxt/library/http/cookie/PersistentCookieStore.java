...
public class PersistentCookieStore {
    ...
    public PersistentCookieStore(Context context) {
        cookiePrefs = context.getSharedPreferences(context.getPackageName() + "_" + COOKIE_PREFS, Context.MODE_PRIVATE);
        cookies = new HashMap<>();
    ...
    private String getKey(HttpUrl url) {
        return url.host() + "_" + url.port();
    }

    public final void add(HttpUrl url, Cookie cookie) {
    ...
    public final boolean remove(HttpUrl url, Cookie cookie) {
        String name = getCookieToken(cookie);
        String key = getKey(url);
        if (cookies.containsKey(key) && cookies.get(key).containsKey(name)) {
            cookies.get(key).remove(name);
            ...
            prefsWriter.putString(url.host(), TextUtils.join(",", cookies.get(key).keySet()));
            prefsWriter.apply();
     ...
     * @return 序列化之后的string
     */
    protected final String encodeCookie(SerializableOkHttpCookies cookie) {
        if (cookie == null)
            return null;
